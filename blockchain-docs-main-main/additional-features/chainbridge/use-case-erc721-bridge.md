...
### Scenario
This scenario is to setup a Bridge for the ERC721 NFT that has been deployed in public chain (EVMBuilder PoS) already in order to enable low-cost transfer in a private chain (EVMBuilder Edge) for users in a regular case. In such a case, the original metadata has been defined in the public chain and the only NFTs that have been transferred from Public chain can exist in the private chain. For that reason, you'll need to use lock/release mode in the public chain and burn/mint mode in the private chain.
...
```
# Deploy Bridge and ERC721 contracts in EVMBuilder PoS chain
...
```
# Deploy Bridge and ERC721 contracts in EVMBuilder Edge chain
...
You'll deploy your ERC721 contract. This example guides you with hardhat project [Trapesys/chainbridge-example](https://github.com/EVMBuilder-Smartchain/chainbridge-example).
...
Before starting this step, please make sure that you've started relayer. Please check [Setup](setup.md) for more details.
...
$ cb-sol-cli erc721 deposit \
  ...
  --gasPrice [GAS_PRICE] \
  --id 0x50 \
  # ChainID for EVMBuilder Edge chain
  --dest 100 \
  --bridge "[BRIDGE_CONTRACT_ADDRESS]" \
...
It executes a transaction to send NFT to the recipient account in EVMBuilder Edge chain after the required number of votes are submitted.
...
Once the execution transaction is successful, you will get NFT in EVMBuilder Edge chain.
