...
In this guide, you will use Mumbai EVMBuilder PoS testnet and EVMBuilder Edge local chain. Please make sure you have JSON-RPC endpoint for Mumbai and you've set up EVMBuilder Edge in local environment. Please refer to [Local Setup](../../get-started/local-setup.md) or [Cloud Setup](../../get-started/cloud-setup.md) for more details.

### Scenario

This scenario is to setup a Bridge for the ERC20 token that has been deployed in public chain (EVMBuilder PoS) already in order to enable low-cost transfer in a private chain (EVMBuilder Edge) for users in a regular case. In such a case, the total supply of token has been defined in the public chain and only the amount of the token which has been transferred from the public chain to the private chain must exist in the private chain. For that reason, you'll need to use lock/release mode in the public chain and burn/mint mode in the private chain.
...
All codes and scripts are in Github Repo [Trapesys/chainbridge-example](https://github.com/EVMBuilder-Smartchain/chainbridge-example).
...
```
# Deploy Bridge and ERC20 contracts in EVMBuilder PoS chain
...
```
# Deploy Bridge and ERC20 contracts in EVMBuilder Edge chain
...
You'll deploy your ERC20 contract. This example guides you with hardhat project [Trapesys/chainbridge-example](https://github.com/EVMBuilder-Smartchain/chainbridge-example).
...
Bridge expects to work as burn/mint mode in EVMBuilder Edge. You'll set burn/mint mode using `cb-sol-cli`.
...
Before starting this step, please make sure that you've started a relayer. Please check [Setup](setup.md) for more details.
...
```
# Start transfer from Mumbai to EVMBuilder Edge chain
...
After the deposit transaction is successful, the relayer will get the event and vote for the proposal. It executes a transaction to send tokens to the recipient account in the EVMBuilder Edge chain after the required number of votes are submitted.

```
INFO[11-19|08:15:58] Handling fungible deposit event          chain=mumbai dest=100 nonce=1
INFO[11-19|08:15:59] Attempting to resolve message            chain=Z-edge type=FungibleTransfer src=99 dst=100 nonce=1 rId=000000000000000000000000000000c76ebe4a02bbc34786d860b355f5a5ce00
INFO[11-19|08:15:59] Creating erc20 proposal                  chain=Z-edge src=99 nonce=1
INFO[11-19|08:15:59] Watching for finalization event          chain=Z-edge src=99 nonce=1
INFO[11-19|08:15:59] Submitted proposal vote                  chain=Z-edge tx=0x67a97849951cdf0480e24a95f59adc65ae75da23d00b4ab22e917a2ad2fa940d src=99 depositNonce=1 gasPrice=1
INFO[11-19|08:16:24] Submitted proposal execution             chain=Z-edge tx=0x63615a775a55fcb00676a40e3c9025eeefec94d0c32ee14548891b71f8d1aad1 src=99 dst=100 nonce=1 gasPrice=5
```

Once the execution transaction is successful, you will get tokens in the EVMBuilder Edge chain.
